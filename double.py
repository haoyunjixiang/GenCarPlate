...
import cv2
def makecarnum(str):
    fontchinese = ImageFont.truetype('simhei.ttf',220)#使用自定义的字体，第二个参数表示字符大小
    font = ImageFont.truetype('font.ttf', 280)
    font2 = ImageFont.truetype('font.ttf', 360)
    im = Image.open('2.bmp')
    draw = ImageDraw.Draw(im)#绘图句柄

    xc,yc=(325,35) #初始左上角的坐标
    x, y = (625, 0)

    x2,y2=(255,230)
    draw.text((xc,yc), str[0], font=fontchinese,fill = "black")#绘图M
    offsetx, offsety = fontchinese.getoffset(str[0])  # 获得文字的offset位置
    width, height = fontchinese.getsize(str[0])  # 获得文件的大小
    draw.text((offsetx+x+width, y), str[1:2], font=font,fill = "black")

    draw.text((x2, y2), str[2:], font=font2, fill="black")
    im = np.array(im)
...
englishchars=[u"A",u"B", u"C", u"D", u"E", u"F", u"G", u"H", u"J", u"K", u"L", u"M", u"N", u"P", u"Q", u"R", u"S", u"T", u"U", u"V", u"W", u"X",u"Y", u"Z"]
print(np.random.randint(0, 2))
str=''
for i in range(7):
    if i==0:
        str=str+chinachars[np.random.randint(0, 49)]
    elif i==1:
        str = str + englishchars[np.random.randint(0, 24)]
    else:
        str = str +chars[np.random.randint(0, 34)]
makecarnum(str)